...
const MenuOptions = ({ icon, link, text, sub, newTab, sidebarToggle, mobileView, badge, ...props }) => {
  let currentUrl;
  ...
  const menuHeight = (el) => {
    ...
    for (var i = 0; i < el.length; i++) {
      ...
      var padding =
        parseInt(window.getComputedStyle(el[i]).paddingTop.slice(0, -2)) + parseInt(window.getComputedStyle(el[i]).paddingBottom.slice(0, -2));
  ...
  return (
    <li className={menuItemClass} onClick={(e) => toggleActionSidebar(e)}>
      {newTab ? (
        <NavLink to={link} target="_blank" rel="noopener noreferrer" className="nk-menu-link">
          {icon ? (
        ...
        <Link to={`${link}`} className={`nk-menu-link${sub ? ' nk-menu-toggle' : ''}`} onClick={sub ? menuToggle : null}>
          {icon ? (
            <span className="nk-menu-icon">
              <Icon name={icon} />
            </span>
      ...
      {sub ? (
        <div className="nk-menu-wrap">
          <MenuSubOptions sub={sub} sidebarToggle={sidebarToggle} mobileView={mobileView} />
        </div>
      ) : null}
    </li>
  );
